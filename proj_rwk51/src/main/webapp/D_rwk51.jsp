...
<div>
	<form method="post" action="./rwk51_dao.jsp" >  
      ...
      <input type ="submit" value = "4주이상 기록 전체 삭제">
   </form>
</div>
<%
	int rwk51_id = 0;
...
%>
   ...
   %>
      <tr>
         ...
         <td><%=rwk51_heatchk %></td>
         <td>
				<a href="./rwk51_dao.jsp?rwk51_id=<%=rwk51_id %>&actionType=D">삭제</a>
		</td>
      </tr>
...
<br>
<a href="./D_rwk51.jsp?currentPageNo=0&optionchk=<%=optionchk%>">[FIRST]</a>
<%      
   
   if(currentPageNo > 0 ){
%>
<a href="./D_rwk51.jsp?currentPageNo=<%=(currentPageNo-1) %>&optionchk=<%=optionchk%>" >[PRE]</a>
...
%>
 ...
 %>
    <a href="./D_rwk51.jsp?currentPageNo=<%=i %>&optionchk=<%=optionchk%>">[<%=(i+1) %>]</a>
 <%
      }
   }
...
<%
 ...
 %>
 <a href="./D_rwk51.jsp?currentPageNo=<%=(currentPageNo+1)%>&optionchk=<%=optionchk%>">[NXT]</a>
 <%
   }else{
 %>
 [NXT]
 <%
   ...
   con.close();
 %>
 <a href="./D_rwk51.jsp?currentPageNo=<%=(pageCnt-1) %>&optionchk=<%=optionchk%>">[END]</a>
