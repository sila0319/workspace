...
<%
   ...
   case "all":
	   ...
	   break;
   case  "date":
	   sql = "SELECT * FROM rwk51 ORDER BY CAST(rwk51_date AS SIGNED) limit ?,? ";
	   break;
   case "heat": 
...
<br>
<a href="./R_rwk51.jsp?currentPageNo=0&optionchk=<%=optionchk%>">[FIRST]</a>
<%      
   
   if(currentPageNo > 0 ){
%>
<a href="./R_rwk51.jsp?currentPageNo=<%=(currentPageNo-1) %>&optionchk=<%=optionchk%>" >[PRE]</a>
...
%>
 ...
 %>
    <a href="./R_rwk51.jsp?currentPageNo=<%=i %>&optionchk=<%=optionchk%>">[<%=(i+1) %>]</a>
 <%
      }
   }
...
<%
 ...
 %>
 <a href="./R_rwk51.jsp?currentPageNo=<%=(currentPageNo+1)%>&optionchk=<%=optionchk%>">[NXT]</a>
 <%
   }else{
 %>
 [NXT]
 <%
   ...
   con.close();
 %>
 <a href="./R_rwk51.jsp?currentPageNo=<%=(pageCnt-1) %>&optionchk=<%=optionchk%>">[END]</a>
